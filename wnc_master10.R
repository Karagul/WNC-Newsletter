...
library(reshape2)
library(htmltools)
setwd("~/Desktop/Programming/WNC-Newsletter")
source('wnc_source10.R')
getwd()
...
grouped2 <- grouped %>% filter(date >= filter_date & date <= acting_date) %>% group_by(date, group) %>% summarise(sum = sum(unemp))
...
grouped_rate <- grouped %>% filter(date >= filter_date & date <= acting_date) %>% group_by(date, group) %>% summarise(avg = mean(rate))
...
local_rate <- local %>% filter(date >= filter_date & date <= acting_date) %>% group_by(date, name2) %>% summarise(avg = mean(rate))
...
dates <- seq(ymd(filter_date), ymd(paste(as.character(current_year), as.character(endpd), '1', sep = "-" )), by = 'month')

compare_rate <- data.frame('US' = as.numeric(rev(us_rate_series)), 'NC' = as.numeric(rev(nc_rate_series)), 'WNC' = wnc_series, 'date' = dates)
