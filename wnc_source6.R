...
get_agg_unemp <- function(sets){
  ...
  assign('endpd_agg', endpd, envir = .GlobalEnv)
  agg_unemp <- c()
  for(s in sets){
...
make_level_series <- function(){
  ...
  wnc_series <- make_series(unemployment = u, startyr = startyr_agg, endyr = endyr_agg, endpd = endpd_agg)
